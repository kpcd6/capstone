$(window).load(function() {	
	...
	$('#content > ul > li').find('.box').css({height:'0'})
	...
	function open_page(page){
		...
		Cufon.replace('#menu a', { fontFamily: 'Luicda Console', hover:true });
		num=$(page).data('num');
		...
		if (act!='') {
			$(act).find('.box').stop().animate({height:'0'},700,'easeOutCirc', function(){
				$(act).css({display:'none'});
				$(page).css({display:'block'}).find('.box').stop().animate({height:'100%'},700, 'easeOutCirc', function(){
					act=page;	
				});	
			})
		} else {
